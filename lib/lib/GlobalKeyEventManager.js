...
import AbstractKeyEventManager from './AbstractKeyEventManager';
...
class GlobalKeyEventManager extends AbstractKeyEventManager {
  /**
  ...
  handleKeydown(event, options) {
    ...
    if (reactAppHistoryWithEvent === 'handled') {
      ...
      if (reactAppHistoryWithEvent === 'seen') {
        this.logger.debug(`${this._logPrefix()} Seen '${_key}' keydown event (that has already passed through React app).`);

      } else {
        KeyEventCounter.incrementId();

        this.logger.debug(`${this._logPrefix()} Seen new '${_key}' keydown event (that has NOT passed through React app).`);
      }
  ...
  handleKeypress(event, options) {
    ...
    if (reactAppHistoryWithEvent === 'handled') {
      ...
      if (reactAppHistoryWithEvent === 'seen') {
        this.logger.debug(`${this._logPrefix()} Seen '${_key}' keypress event (that has already passed through React app).`);

      } else {
        KeyEventCounter.incrementId();

        this.logger.debug(`${this._logPrefix()} Seen new '${_key}' keypress event (that has NOT passed through React app).`);
      }
  ...
  handleKeyup(event, options) {
    ...
    if (reactAppHistoryWithEvent === 'handled') {
      ...
      if (reactAppHistoryWithEvent === 'seen') {
        this.logger.debug(`${this._logPrefix()} Seen '${_key}' keyup event (that has already passed through React app).`);

      } else {
        KeyEventCounter.incrementId();

        this.logger.debug(`${this._logPrefix()} Seen new '${_key}' keyup event (that has NOT passed through React app).`);
      }
  ...
  _callHandlerIfActionNotHandled(event, keyName, eventBitmapIndex) {
    ...
    if (this.keyMapEventBitmap[eventBitmapIndex]) {
      ...
      this._callMatchingHandlerClosestToEventTarget(event, keyName, eventBitmapIndex, this.componentList.length);
    } else {
      this.logger.verbose(`${this._logPrefix()} Ignored '${combinationName}' ${eventName} because it doesn't have any ${eventName} handlers.`);
    }
  }

  _logPrefix(componentIndex = undefined) {
    const eventIcons = this.constructor.eventIcons;
    const componentIcons = this.constructor.componentIcons;

    const prefixBase = `HotKeys (GLOBAL-E${KeyEventCounter.getId()}${eventIcons[KeyEventCounter.getId() % eventIcons.length]}`;

    if (isUndefined(componentIndex)) {
      return `${prefixBase}):`;
    } else {
      return `${prefixBase}-C${componentIndex}${componentIcons[componentIndex % componentIcons.length]}):`
    }
  }
}

export default GlobalKeyEventManager;
