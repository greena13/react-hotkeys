...
class KeyEventManager {
  ...
  _buildKeyMatcherMap(actionNameToKeyMap, componentIndex) {
    return Object.keys(actionNameToKeyMap).reduce((keyMapMemo, actionName) => {
      ...
      keyMapMemo[actionName] = keyMapOptions.reduce((sequenceMemo, keyMapOption) => {
        ...
        const { sequence: _keyCombinations, id: keySequenceID, score } = KeySerializer.parseString(keySequence, { eventBitmapIndex });

        if (_keyCombinations.length > this.longestKeySequence) {
          this.longestKeySequence = _keyCombinations.length;
        }

        sequenceMemo[keySequenceID] = {
          id: keySequenceID,
          componentIndex,
  ...
  _addToCurrentKeyCombination(keyName, bitmapIndex) {
    ...
    const existingBitmap = keyCombination[keyName];
    ...
    if (!existingBitmap) {
      ...
      delete keyCombination[keyName][0];
  ...
  _withoutKeyUps(keyCombinationRecord) {
    return Object.keys(keyCombinationRecord).reduce((memo, keyName) => {
      const keyState = keyCombinationRecord[keyName];
  ...
  _callHandlerIfActionNotHandled(event, keyName, eventBitmapIndex, componentIndex) {
    if (!this.eventPropagationState.actionHandled) {
      this._callHighestPriorityHandler(event, keyName, eventBitmapIndex, componentIndex);
    }
  }

  _callHighestPriorityHandler(event, keyName, eventBitmapIndex, componentIndex) {
    ...
    const previousKeyMatch = this.eventPropagationState.previousKeyMatcher;
     ...
     */
    let counter = this.keyCombinationHistory.length;

    console.warn('keyMatcher.sequences:');
    console.warn(keyMatcher.sequences);

    while(counter >= 0) {
      const combinationState = this.keyCombinationHistory.slice(-counter);

      const keySequenceString = KeySerializer.sequence(combinationState);

      const matchingSequence = keyMatcher.sequences[keySequenceString];

      if (matchingSequence) {
        if (this._combinationMatchesKeys(keyName, currentKeyCombination, indexFromEnd(matchingSequence.sequence, 0))) {
    ...
    // let candidateCombinationMatchIndex = 0;
    //
    // while (candidateCombinationMatchIndex < keyMatcher.combinationsOrder.length) {
    //   candidateCombinationMatchIndex = this._longestMatchingCombinationIn(
    //     currentKeyCombination,
    //     keyMatcher,
    //     keyName,
    //     candidateCombinationMatchIndex
    //   );
    //
    //   if (candidateCombinationMatchIndex === -1) {
    //     /**
    //      * No matching combination was found
    //      */
    //
    //     this.eventPropagationState.previousKeyMatcher = null;
    //     return null;
    //
    //   } else {
    //     /**
    //      * A matching combination was found
    //      */
    //     const combinationId = keyMatcher.combinationsOrder[candidateCombinationMatchIndex];
    ...
    //     if (handlerFound) {
    //       /**
    //        * If the matching combination has a handler defined, call it and stop looking.
    //        */
    //
    //       this.eventPropagationState.previousKeyMatcher = combinationId;
    //
    //       return keyMatcher.combinations[combinationId].actionName;
    //     } else {
    //       /**
    //        * If the matching combination doesn't have a handler defined, keep looking
    //        * for other, shorter combinations to see if any of them have handlers
    //        * defined
    //        */
    //
    //       candidateCombinationMatchIndex++;
  ...
  _longestMatchingCombinationIn(keyCombination, keyMatcher, keyName, startIndex) {
    let combinationIndex = startIndex;

    while(combinationIndex < keyMatcher.combinationsOrder.length) {
      const combinationId = keyMatcher.combinationsOrder[combinationIndex];
      const candidateKeyCombination = keyMatcher.combinations[combinationId];

      if (this._combinationMatchesKeys(keyName, keyCombination, candidateKeyCombination)) {
  ...
  _combinationMatchesKeys(keyName, keyStates, combinationMatch) {
    ...
    const combinationMatchesKeysPressed = !Object.keys(combinationMatch.keyDictionary).some((candidateKeyName) => {
      ...
      const keyEventBitmap = keyStates[candidateKeyName];
  ...
  _callHandler(event, matchingAction, componentIndex) {
    ...
    return this.eventPropagationState.actionHandled;
  }
  ...
  _buildCumulativeKeyMatchers() {
    ...
    return this.componentList.reduce((memo, { keyMatcher }) => {
      const previousKeyMap = memo[memo.length -1] || { sequences: {} };
       ...
       */
      const sortedCombinations = orderBy(Object.values(combinations), [ 'componentIndex', 'size' ] , [ 'asc', 'desc' ]);
