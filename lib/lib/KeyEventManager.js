...
class KeyEventManager {
  ...
  _callHighestPriorityHandler(event, keyName, eventBitmapIndex, componentIndex) {
    ...
    while(combinationIndex < keyMatcher.combinationsOrder.length) {
      ...
      const combinationMatchesKeysPressed = !Object.keys(candidateKeyCombination.keyDictionary).some((candidateKeyName) => {
        const candidateBitmapIndex = candidateKeyCombination.eventBitmapIndex;

        const keyEventBitmap = currentKeyCombination[candidateKeyName];

        if (keyEventBitmap) {
          const keyEventBitTrue = keyEventBitmap[KeyEventBitmapIndex.current][candidateBitmapIndex];

          if (candidateKeyName === keyName) {
      ...
      if (combinationMatchesKeysPressed && keyCompletesCombination) {
