...
class KeyEventManager {
  ...
  _buildComponentOptions(componentIndex, actionNameToKeyMap, actionNameToHandlersMap) {
    ...
    return {
      ...this._buildKeyMatcherMap({ ...actionNameToKeyMap, ...hardSequenceKeyMap }, componentIndex),
      handlers
    };
  }
  ...
  _combinationMatchesKeys(keyName, keyState, combinationMatch) {
    ...
    const combinationMatchesKeysPressed = !Object.keys(combinationMatch.keyDictionary).some((candidateKeyName) => {
      ...
      const candidateKeyNames = function(){
        if (combinationIncludesShift) {
          return [ candidateKeyName, ...resolveShiftAlias(candidateKeyName) ];
        } else {
          return [ candidateKeyName, ...resolveKeyAlias(candidateKeyName) ];
        }
      }();

      const matchingKeyName = candidateKeyNames.find((keyName) => keyState.keys[keyName]);

      if (matchingKeyName) {
        const keyEventBitmap = keyState.keys[matchingKeyName];
        const keyEventBitTrue = keyEventBitmap[KeyEventBitmapIndex.current][candidateBitmapIndex];

        if (keyName && matchingKeyName === keyName) {
