...
class KeyEventManager {
  ...
  _reset() {
     ...
     * @type {Number[]}
     */
    this.unmatchedHandlerCounts = null;
     ...
     * state.
     * @type {{keyStateIncludesKeyUp: boolean}}
     */
    this.flags = {
      /**
  ...
  handleFocus(actionNameToKeyMap = {}, actionNameToHandlersMap = {}) {
    ...
    const componentOptions = this._buildComponentOptions(componentIndex, actionNameToKeyMap, actionNameToHandlersMap);

    this.componentList.unshift(componentOptions);

    return [this.focusTreeId, componentIndex];
  }

  _buildComponentOptions(componentIndex, actionNameToKeyMap, actionNameToHandlersMap) {
    ...
    return {
      actions: this._buildKeyMatcherMap({ ...actionNameToKeyMap, ...hardSequenceKeyMap }, componentIndex),
      handlers
    };
  }

  updateComponent(focusTreeId, componentIndex, actionNameToKeyMap = {}, actionNameToHandlersMap = {}) {
    if (focusTreeId !== this.focusTreeId || !this.componentList[componentIndex]) {
      return;
    }

    this.componentList[componentIndex] = this._buildComponentOptions(componentIndex, actionNameToKeyMap, actionNameToHandlersMap);
  }
        ...
        const { keySequence, eventBitmapIndex } = function(){
          if (isObject(keyMapOption)) {
            ...
            return {
              ...
              eventBitmapIndex: isUndefined(action) ? KeyEventBitmapIndex.keypress : KeyEventBitmapIndex[action]
            };
          } else {
            return {
  ...
  handleBlur(focusTreeId, componentIndex){
    ...
    return this.eventPropagationState.previousComponentIndex > componentIndex && componentIndex < this.componentList.length - 1;
  }
   ...
   */
  handleKeyDown(event, focusTreeId, componentIndex) {
    if (focusTreeId !== this.focusTreeId) {
      return true;
    }
   ...
   *        the keyboard event as it bubbles towards the document root.
   */
  handleKeyPress(event, focusTreeId, componentIndex) {
    if (focusTreeId !== this.focusTreeId) {
      return true;
    }
   ...
   */
  handleKeyUp(event, focusTreeId, componentIndex) {
    if (focusTreeId !== this.focusTreeId) {
      return true;
    }
  ...
  _callMatchingHandlerClosestToEventTarget(event, keyName, eventBitmapIndex, componentIndex) {
    if (!this.keyMaps || !this.unmatchedHandlerCounts) {
      /**
       * Initialize key maps and unmatched handler counts the first time a key event
       * is received by a new focus tree
       */
      this.keyMaps = [];
      this.unmatchedHandlerCounts = [];

      this.componentList.forEach(({ handlers }) => {
        this.unmatchedHandlerCounts.push(Object.keys(handlers).length);
        this.keyMaps.push({});
      });
    }

    const unmatchedHandlersCount = indexFromEnd(this.unmatchedHandlerCounts, componentIndex);
    ...
    if (unmatchedHandlersCount > 0) {
      ...
      while (this.searchIndex < this.componentList.length && unmatchedHandlersCount > 0) {
        ...
        const { handlers, actions } = indexFromEnd(this.componentList, this.searchIndex);
        ...
        Object.keys(actions).forEach((actionName) => {
          ...
          if (handlerComponentIndexes) {
            const keyMatcher = actions[actionName];
            ...
            const handler = indexFromEnd(this.componentList, handlerComponentIndex).handlers[actionName];

            /**
             * Action has a handler defined, that needs connecting
             */
            const keyMap = indexFromEnd(this.keyMaps, handlerComponentIndex);

            /**
             * Store the key sequence with the handler that it should trigger at
            ...
            /**
             * Decrement the number of remaining unmatched handlers for the component
             * currently handling the propagating key event, so we know when all handlers
             * have been matched to sequences and we can move on to matching them against
             * the current key event
             */
            this.unmatchedHandlerCounts[this.unmatchedHandlerCounts.length - (handlerComponentIndex + 1)]--;
          }
        });
    ...
    const keyMap = indexFromEnd(this.keyMaps, componentIndex);
