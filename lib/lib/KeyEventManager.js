...
class KeyEventManager {
  ...
  constructor(configuration = {}) {
    ...
    this.globalComponentDict = {};

    this.globalComponentIndexList = [];

    this.globalKeyEventBitmap = KeyEventBitmapManager.newBitmap();

    this.globalHandlerCounts = {
    ...
    /**
     * Unique index that is incremented and assigned to each new global component
     * that registers itself
     * @type {number}
     */
    this.globalComponentIndex = 0;
  ...
  addGlobalHotKeys(actionNameToKeyMap = {}, actionNameToHandlersMap = {}, options) {
    const componentIndex = this.globalComponentList;

    /**
     * Increment globalComponentIndex so the next global component that is registered
     * will have the next index number
     */
    this.globalComponentIndex += 1;
    ...
    /**
     * Store the component options in a dictionary for ease of retrieval later, if
     * a particular component's hotkeys need to be updated
     */
    this.globalComponentDict[componentIndex] = {
      componentOptions,
      keyMapEventBitmap
    };

    this._updateGlobalHandlerCounts(keyMapEventBitmap, 1);

    /**
     * Store the component index in an array to create an explicit order of traversal
     * for resolving hot key matches
     */
    this.globalComponentIndexList.push(componentIndex);

    this._updateGlobalHandlerCounts(
      KeyEventBitmapManager.newBitmap(),
      this.globalComponentDict[componentIndex].keyMapEventBitmap
    );

    this.logger.debug(
      `Global component ${componentIndex} registered. \nKey maps: `,
      actionNameToKeyMap,
      '\nHandlers: ',
      actionNameToHandlersMap
    );
  ...
  updateGlobalComponent(componentIndex, actionNameToKeyMap = {}, actionNameToHandlersMap = {}, options) {
    const beforeKeyMapEventBitmap = KeyEventBitmapManager.clone(
      this.globalComponentDict[componentIndex].keyMapEventBitmap
    );


    this.globalComponentDict[componentIndex] = this._buildComponentOptions(
      componentIndex,
      actionNameToKeyMap,
      actionNameToHandlersMap,
      {
        ...options,
        keyMapEventBitmap: this.globalKeyEventBitmap
      }
    );

    const afterKeyMapEventBitmap = this.globalComponentDict[componentIndex].keyMapEventBitmap;

    this._updateGlobalHandlerCounts(beforeKeyMapEventBitmap, afterKeyMapEventBitmap);
  ...
  removeGlobalComponent(componentIndex) {
    const keyMapEventBitmap = this.globalComponentDict[componentIndex].keyMapEventBitmap;

    this._updateGlobalHandlerCounts(keyMapEventBitmap, KeyEventBitmapManager.newBitmap());

    this.globalComponentIndexList = without(this.globalComponentIndexList, componentIndex);
    delete this.globalComponentDict[componentIndex];
  }

  _updateGlobalHandlerCounts(beforeKeyMapEventBitmap, afterKeyMapEventBitmap){
    Object.keys(this.globalHandlerCounts).forEach((keyMapEventBitmapIndex) => {
      const handlerCountBefore = this.globalHandlerCounts[keyMapEventBitmapIndex];

      if (beforeKeyMapEventBitmap[keyMapEventBitmapIndex]) {
        if (!afterKeyMapEventBitmap[keyMapEventBitmapIndex]) {
          /**
           * A handler for the specified key event has been removed, so we
           * decrement the counter
           */
          this.globalHandlerCounts[keyMapEventBitmapIndex] -= 1;
        }
      } else {
        if (afterKeyMapEventBitmap[keyMapEventBitmapIndex]) {
          /**
      ...
      const handlerCountAfter = this.globalHandlerCounts[keyMapEventBitmapIndex];
      ...
      if (handlerCountBefore === 0 && handlerCountAfter === 1) {
        ...
        const eventName = this.constructor._describeKeyEvent(keyMapEventBitmapIndex);

        document[`on${eventName}`] = (keyEvent) => {
           this._handleGlobalKeyEvent(keyMapEventBitmapIndex);
        }
  ...
  handleFocus(actionNameToKeyMap = {}, actionNameToHandlersMap = {}, options) {
    ...
    this.logger.debug(`${this._logPrefix(componentIndex)} Focused. \nKey maps: `, actionNameToKeyMap, '\nHandlers: ', actionNameToHandlersMap);
  ...
  updateComponent(focusTreeId, componentIndex, actionNameToKeyMap = {}, actionNameToHandlersMap = {}, options) {
    ...
    this.logger.verbose(`${this._logPrefix(componentIndex, focusTreeId)} Received new props: \nKey maps: `, actionNameToKeyMap, '\nHandlers: ', actionNameToHandlersMap);
  }
  ...
  _buildKeyMatcherMap(actionNameToKeyMap, componentIndex, options) {
    return Object.keys(actionNameToKeyMap).reduce((keyMapMemo, actionName) => {
      ...
      keyMapOptions.forEach((keyMapOption) => {
         ...
         * the keyboard event
         */
        options.keyEventBitmap[eventBitmapIndex] = true;
  ...
  _logPrefix(componentIndex, focusTreeId = this.focusTreeId) {
    ...
    return `HotKeys (FT${focusTreeId}${logIcons[focusTreeId % logIcons.length]}-E${this.eventId}${eventIcons[this.eventId % eventIcons.length]}-C${componentIndex}${componentIcons[componentIndex % componentIcons.length]}):`
  }
  ...
  handleKeyUp(event, focusTreeId, componentIndex, options) {
    ...
    if (this._isNewKeyEvent(componentIndex)) {
      ...
      if (alreadySeenKeyInCurrentCombo) {
        ...
        this.logger.debug(`${this._logPrefix(componentIndex, focusTreeId)} Added ${_key} to current combination: ${this._describeCurrentKeyCombination()}.`);
  ...
  _setNewEventParameters(event, type) {
    ...
    this.eventPropagationState.key = event.key;
    this.eventPropagationState.type = type;
  }
